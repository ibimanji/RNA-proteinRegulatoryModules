...
ComputeCellSpecificAlpha <- function(
    embedding_list,
    k = 30,
    temperature = 1
){
  ...
  if(is.null(names(embedding_list))){
    stop(
      "embedding_list must be named"
    )
  }
  
  
  modality_names <- names(embedding_list)
  ...
  for(m in modality_names){
    ...
    if(
      nrow(embedding_list[[m]]) != n
    ){
      stop(
        "All embeddings must have same number of cells"
      )
    }
  }
  ...
  alpha_score <-
    matrix(
      0,
      nrow=n,
      ncol=length(modality_names)
    )
  
  
  colnames(alpha_score)<-
    modality_names
  ...
  for(target_modality in modality_names){
    
    
    X_target <-
      embedding_list[[target_modality]]
    ...
    ################################
    ## every modality predicts target
    ################################
    
    
    for(source_modality in modality_names){
      
      
      X_source <-
        embedding_list[[source_modality]]
      
      
      knn_idx <-
        knn_list[[source_modality]]
      ...
      for(i in seq_len(n)){
        ...
        pred[i,] <-
          colMeans(
            X_target[neighbors,,drop=FALSE]
          )
      }
      
      
      error_mat[,source_modality] <-
        sqrt(
          rowSums(
            (
              X_target - pred
    ...
    score <-
      affinity[,target_modality] /
      rowSums(affinity)
    
    
    alpha_score[,target_modality] <-
      score
  ...
  ####################################
  ## final softmax across modalities
  ####################################
  
  
  alpha_cell <-
    exp(alpha_score) /
    rowSums(exp(alpha_score))
...
############################################################
## Cell-wise Modality Spectral Energy after SCML
##
## Input:
##   scml_result from run_scml_core_pan()
##
## Output:
##   cell x modality spectral energy matrix
############################################################


ComputeCellModalitySpectralEnergy <- function(
    scml_result,
    remove_first = TRUE,
    use_raw_U = TRUE,
    absolute = TRUE
  ...
  ##########################################################
  ## 1. obtain consensus eigenvectors
  ##########################################################
  
  if(use_raw_U && !is.null(scml_result$U_raw)){
  ...
  ##########################################################
  ## 2. remove first eigenvector
  ##    remove graph DC component
  ##########################################################
  ...
  L_list <- scml_result$laplacians
  
  
  if(is.null(L_list)){
    
    stop(
      "laplacians not returned. Please run SCML with return_laplacians=TRUE"
    )
  ...
  ##########################################################
  ## 5. modality-wise spectral energy
  ##########################################################
...
NormalizeSpectralEnergy <- function(
    Energy,
    method=c(
      "zscore",
      "rank",
...
EnergyToAlpha <- function(
  ...
  ####################################################
  ## set removed modality energy to zero
  ####################################################
  
  if(!is.null(remove)){
    
    
    if(!all(remove %in% colnames(Energy_use))){
      
      stop(
        "Unknown modality in remove: ",
        paste(
          setdiff(remove, colnames(Energy_use)),
          collapse=", "
        )
      )
...
BuildCellAdaptiveProjection <- function(
    U,
    alpha_cell
){
...
run_scml_core_matrixalpha <- function(
    graph_list,
    ndim = 30,
    alpha = NULL,
    layer_weight = NULL,
  ...
  if(is.matrix(alpha)){
    ...
    if(
      ncol(alpha)!=length(layer_names)
    ){
      stop(
        "Matrix alpha must have one column per layer"
      )
    }
...
MinMaxNormalizeByReference <- function(
    norm,
    imp
){
  
  stopifnot(
    all(dim(norm)==dim(imp))
  )
  
  stopifnot(
    identical(rownames(norm), rownames(imp))
  )
  
  stopifnot(
    identical(colnames(norm), colnames(imp))
  )
  
  
  ########################################
  # range of norm (target)
  ########################################
